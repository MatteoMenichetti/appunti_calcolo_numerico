...
Saranno trattati i due casi seguenti di rappresentazione in aritmetica finita di una quantita' numerica:
\begin{itemize}
	\item numeri interi,
	\item numeri reali.
\end{itemize}
...
\begin{definition}[Notazione scientifica normalizzata]
	La stringa (\ref{eq:rappresentazione_numero_reale}), sotto la condizione $\alpha_1\neq 0$, e' rappresentabile in modo unico in notazione scientifica normalizzata in base $b$ con la seguente forma
...
\paragraph{Cosa rappresenta la precisione macchina $\boldsymbol{u}$?}
...
\begin{remark}[\textbf{Precisione macchina singola precisione}]
	Dato che la rappresentazione è per arrotondamento allora la precisione macchina può essere approssimata come $\boldsymbol{u\approx 2^{-24}}$ (vedere il Teorema \ref{th:precisione_macchina_FL})
\end{remark}
