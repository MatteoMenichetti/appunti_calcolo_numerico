...
\begin{theorem}\label{th:matrice_diagonale_dominante_nonsingolare}\footnote{Slide 5 PDF 5, Lemma 3.9 PG 55.}
    Se $A\in\mathbb{R}^{n\times n}$ è \textit{d.d.} per righe (o per colonne) $\Rightarrow A$ \gls{nonsingolare} ($\rightarrow A$ è fattorizzabile $LU$).
\end{theorem}
\begin{proof}
    ...
    \end{equation}
    con $x_i$ la componente $i$-esima di $\uline x$. Allora, è esaminata la $\boldsymbol{k}$\textbf{-esima} equazione del sistema $A\uline x=\uline 0.$ Formalmente, moltiplicando, membro a membro, per $\boldsymbol{\uline e_k^T}$, il trasposto del $k$-esimo versore della base canonica
    \begin{equation*}
    	\underbrace{\left(\uline e_k^T A\right)}_{\footnotemark}\uline x=\uline e_k^T\uline 0\boldsymbol{=0},
    \end{equation*}
    è ottenuto
    \begin{equation*}
