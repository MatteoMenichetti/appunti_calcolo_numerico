...
\begin{definition}[Matrice nonsingolare]\label{def:matrice_nonsingolare}
    ...
    \begin{equation}\label{eq:soluzione_sistema_quadrato}
        \uline x = A^{-1} \uline b.
    \end{equation}
\end{definition}
La soluzione (\ref{eq:soluzione_sistema_quadrato}) è una risoluzione del problema che non definisce, in genere, un algoritmo efficiente di risoluzione dal punto di vista computazionale.
...
\label{alg:fattorizzazione_LDL}
    \begin{lstlisting}[style=Matlab-editor]
        function A = fattorizzazione_LDLT(A)
        %
        % function A = fattorizzazione_LDL(A)
        % Function che restituisce la matrice fattorizzata LDL'. Sulla
        % diagonale della matrice restituita e' presente D, mentre la sezione
        % triangolare inferiore conterra' la matrice triangolare inferiore a
        % diagonale unitaria L.
        %
        % Input:
        ...
        if nargin < 1, error('argomento non presente'), end
        ...
        for j = 2 : n
	        ...
	        if A(j,j) <= 0, error('matrice non sdp'), end
...
È ricercata la soluzione ai minimi quadrati del sistema lineare sovradimensionato (\ref{eq:sistema_lineare_equazioni_sovradimensionato}), la soluzione definita come Definizione \ref{def:soluzione_minimi_quadrati}, che minimizza $ ||\uline r||_2^2$, ovvero:
...
Per derivare il costo della fattorizzazione $QR$ è esaminato lo pseudo-codice Matlab che implementa l'algoritmo in questione, ovvero l'Algoritmo \ref{alg:fattorizzazione_QR}. È assunto che $A\in\mathbb R^{m\times n},$ con $m>n,$ la quale sarà sovrascrita con l'informazione della sua fattorizzazione $QR.$

\begin{algorithm}
\caption{Fattorizzazione $QR$ di Householder.}
\label{alg:fattorizzazione_QR}
    \begin{lstlisting}[style=Matlab-editor]
        function A = QRfact(A)
        [m,n] = size(A);
        for i = 1 : n
...
Dato l'Algoritmo \ref{alg:fattorizzazione_QR} è possibile affermare che il \textbf{costo dell'algoritmo di fattorizzazione} $\boldsymbol{QR}$ è il seguente:
\begin{equation*}
    \sum_{i=1}^n\underbrace{2\times 2(m-i)(n-i)}_{\footnotemark}\approx \frac{2}{3}n^2 (3m-n) \;flops.
\end{equation*}
\footnotetext{Data l'ultima riga dell'Algoritmo \ref{alg:fattorizzazione_QR} allora è possibile affermare che il primo 2 sia il costo dovuto alla sottrazione, $(m-i)$ è costo di A(i:m, i), quindi il $2(m-i)$ è dovuto al calcolo due volte e $2(m-i)(n-i)$ è il costo delle moltiplicazioni.}

A riguardo l'Algoritmo \ref{alg:fattorizzazione_QR} è possibile affermare quanto segue:
\begin{itemize}
...
Con sistemi nonlineari si intende equazioni del tipo $F(\uline x) = \uline 0$ dove $F(\uline x)$ è un vettore di equazioni nonlineari. Per determinare la radice di un'equazione scalare, del tipo $f(x)=0$, il metodo di Newton è uno dei metodi più efficaci. Questo metodo è formalmente definito dall'espressione funzionale (\ref{eq:approssimazione_newton}), a partire da un'approsimazione $x_0$ e può essere esteso agevolmente anche al caso di \textbf{sistemi di equazioni nonlineari.} In questo caso, definite
\begin{equation}
    \uline x =
    \begin{pmatrix}
        x_1\\
...
\begin{equation}\label{eq:metodo_newton_sistema_nonlineare}
...
\paragraph{N.B.:} Tuttavia, l'equazione (\ref{eq:metodo_newton_sistema_nonlineare}) è equivalente, moltiplicando membro a membro per $F'(\uline x_n)$, a
\begin{equation*}
    F'(\uline x_n)\uline x_{n+1} = F'(\uline x_n)\uline x_n - F(\uline x_n),
\end{equation*}
ovvero, ponendo $\Delta\uline x_n = \uline x_{n+1} - \uline x_n,$
\begin{equation}\label{eq:sistema_equazioni_nonlineari}
    \begin{cases}
        F'(\uline x_n) \Delta\uline x_n = - F(\uline x),\\
        \uline x_{n+1} = \uline x_n + \Delta\uline x_n, & n=0,1,\hdots
    \end{cases}
\end{equation}
dove $F'(\uline x_n)$ è una matrice conosciuta ed $\Delta\uline x_n$ è un vettore sconosciuto. Pertanto, risolvere (\ref{eq:sistema_equazioni_nonlineari}) significa risolvere \textbf{per} $\boldsymbol{n=0,1,2,\hdots}$ il sistema lineare $F'(\uline x_n) \Delta\uline x_n = - F(\uline x_n)$, tramite il metodo di Newton, aggiornando $\uline x_{n+1} = \uline x_n + \Delta\uline x_n$.
...
\paragraph{Criterio d'arresto:} Il criterio di arresto di Newton (\ref{eq:criterio_arresto_newton}) può essere generalizzato dal caso scalare con:
\begin{equation}
    ||\Delta x.\slash(1+|x|)||_\infty\leq tol.
\end{equation}

\paragraph{Costo computazionale:} Il costo per iterazione di (\ref{eq:sistema_equazioni_nonlineari}) ammonta a:
\begin{enumerate}
...
\end{enumerate}
I punti 2. e 3. sono i più onerosi dell'algoritmo e pertanto sono sostenuti una sola volta, prima di iniziare l'iterazione. Per questo motivo, questa iterazione è molto utilizzata nelle applicazioni. Per semplificare questo costo è possibile fare ricorso al metodo delle corde (o Newton semplificato, vedere Sezione \ref{sssec:metodo_corde}), la cui iterazione è:
\begin{equation*}
    \begin{cases}
        \boldsymbol{F'(\uline x_0)}\Delta\uline x_n = - F(\uline x_n)\\
        \uline x_{n+1} = \uline x_n + \Delta\uline x_n, & n=0,1,2,\hdots,
    \end{cases}
\end{equation*}
dove $\boldsymbol{F'(\uline x_0)}$ è calcolata solo una volta, ovvero nell'approssimazione iniziale $x_0$.

\paragraph{Implementazione:} Vedere Algoritmo \ref{alg:soluzione_sistema_nonlineare_newton}.

\begin{algorithm}
	\caption{Risoluzione di un sistema nonlineare tramite Newton.}
	\label{alg:soluzione_sistema_nonlineare_newton}
	\begin{lstlisting}[style=Matlab-editor]
		function [nit, xstar] = newton_nonlineare(f,jacobian,x0,tol,itmax)
		%
		% [x,nit] = newton_nonlineare(fun,jacobian,x0,tol,maxit) 
		% Risolve i sistemi di equazioni non lineari tramite il metodo di Newton
		%
		% Input:
		%   - f: matrice di funzioni
		%   - jacobian: matrice jacobiana
		%   - x0: punto iniziale
		%   - tol: tolleranza dell'errore (default eps*10)
		%   - itmax: numero massimo d'iterazioni (deafult 1000)
		% Output:
		%   - x: soluzione del sistema
		%   - nit: numero d'iterazioni impiegate
		% 
		
		if nargin < 3, error('numero argomenti insufficiente'), end
		...
		if itmax <= 0, error('iterazione massime negative'), end
		
		[m,n] = size(jacobian);
		
		if m ~= n, error('jacobiana non quadrata'),end
		
		for nit = 1 : itmax
		fx = -feval(f,x0);
		
		j0 = feval(jacobian,x0);
		
		xn = risoluzione_sistema(j0, fx); % esempio: fattorizzazione LU per poi risolvere il sistema
		
		err = norm((xn./(1+abs(x0))), Inf);
		...
		if err>tol, error("metodo non convergente"); end
