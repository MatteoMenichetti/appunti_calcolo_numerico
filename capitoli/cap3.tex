...
\begin{theorem}[Esistenza della fattorizzazione $LU$]\label{th:esistenza_fattorizzazione_LU}\footnote{Slide 10 PDF 4, Teorema 3.2 PG 52.}\label{th:esFattLU}
...
\begin{remark}
    Il Teorema \ref{th:esFattLU} è una proprietà restrittiva ($A$ \gls{nonsingolare}), la quale sarà ereditata dalle sottomatrici di ordine $k$ di $A$.
\end{remark}

\subsection{Matrici a diagonale dominante}\label{ssec:matrici_diagonale_dominante}
In questa Sezione e nella \ref{ssec:matrice_SDP_fattorizzabile_LDL} sarà visto come matrici delle classi trattate soddisfino il Teorema \ref{th:esistenza_fattorizzazione_LU} di Esistenza della fattorizzazione $LU$. Il fatto che valga il Teorema è dovuto da quanto segue:
\begin{itemize}
...
\begin{theorem}\label{th:matrice_diagonale_dominante_nonsingolare}\footnote{Slide 5 PDF 5, Lemma 3.9 PG 55.}
    $A$ \textit{d.d.} (per righe o per colonne) $\Rightarrow A$ \gls{nonsingolare} ($\rightarrow A$ è fattorizzabile $LU$).
\end{theorem}
\begin{proof}
    ...
    \end{equation*}
    Questo contraddice l'ipotesi che $A$ sia \textit{d.d.} per righe. L'assurdo è aver assunto che $A$ fosse singolare, è concluso che $A$ deve essere \gls{nonsingolare}.
\end{proof}
...
\end{equation*}
\footnotetext{$A$ \gls{nonsingolare}, se fosse singolare allora il problema sarebbe mal posto.}
