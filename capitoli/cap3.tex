...
\begin{definition}[Matrice nonsingolare]
    La matrice $A\in\mathbb R^{n\times n}$, quadrata, è nonsingolare se $rank(A)=n$ (\textbf{quindi} $\boldsymbol{\det(A)\neq 0}$) ed il sistema (\ref{eq:sistEqLin}) ha soluzione unica del tipo
...
Esaminando il costo computazionale per risolvere il sistema lineare (\ref{eq:sistEqLin}), mediante (\ref{eq:soluzione_sistema_triangolare_inferiore}), allora: per calcolare il numeratore sono necessarie $i-1$ moltiplicazioni, $i-1$ somme ed $1$ divisione finale (per calcolare $x_i$) per un totale di $2i-1$ \textit{flops}, quindi il costo totale è dato da \begin{equation*}
