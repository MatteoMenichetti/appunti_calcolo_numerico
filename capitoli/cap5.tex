...
\begin{definition}[Formula di Newton-Cotes]
	...
	\begin{equation}\label{eq:formula_newton_cotes}
		I(f)\approx I(p_n) = \int_a^b p(x)dx=\underbrace{\sum_{i=0}^n f_i\int_a^b L_{in}(x)dx}_{\footnotemark}\equiv I_n(f),
...
\begin{enumerate}
    \item $\boldsymbol{\underline{n=1}}$: dalle Proprietà (\ref{eq:coefficienti_simmetrici_formula_newton_cotes}) e  (\ref{eq:coefficienti_formula_newton_cotes_uguale_1})segue che $c_{01}=c_{11}=\frac{1}{2}$. Pertanto, quando il grado è 1, \textbf{è definita la formula dei trapezi}.
...
Data una perturbazione $\Tilde{f}(x)$ di $f(x)$, con $c_{in}$ definita come in (\ref{eq:formula_newton_cotes_conveniente}), è possibile studiare come $\Tilde{f}(x)$ influisca su (\ref{eq:formula_trapezi}), studiandone il condizionamento. In analogia con quanto visto per il condizionamento di (\ref{eq:problema_quadratura}), è ottenuta la seguente definizione.
...
\begin{definition}[\textbf{Formula dei trapezi composita}]\footnote{Slide 11 PDF 26.}
    Siano $\{x_i\}$ ascisse equidistanziate in $[a,b]$ definite come in (\ref{eq:condizione_ascisse_equidistanti_N-C}), è ottenuta la \textbf{formula dei trapezi composita}:
...
\begin{remark}
    \footnote{Slide 12 PDF 26.} Se $f(x)$ è periodica allora $f_n=f_0\Rightarrow I_1^{(n)}(f)=h\underset{\footnotemark}{\sum_{i=0}^{n-1}f_i}.$ 
...
\paragraph{Implementazione:} Vedere l'Algoritmo \ref{alg:trapec}.
