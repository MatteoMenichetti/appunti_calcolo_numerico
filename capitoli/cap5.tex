...
\subsubsection{Formula dei trapezi e di Simpson}
...
\subsubsubsection{Formula dei trapezi composita}
...
\subsubsubsection{Formula di Simpson composita}
\begin{definition}[\textbf{Formula di Simpson composita}]
    ...
    \begin{equation}\label{eq:formula_simpson_composita}
        \begin{matrix}
            ...
            &=&\frac{h}{3}\underbrace{\left[4\sum_{i=1}^{\frac{n}{2}}f_{2i-1}+2\sum_{i=0}^{\frac{n}{2}}f_{2i}-f_0-f_n\right]}_{\footnotemark}&\boldsymbol\equiv& \boldsymbol{I_2^{(n)}(f)}.
        \end{matrix}
    \end{equation}
\end{definition}
...
\subsubsection{Formula ed errore di quadratura con \texorpdfstring{$\boldsymbol n$}{n} multiplo di \texorpdfstring{$\boldsymbol k$}{k}}
La procedura che porta alle formule dei Trapezi e di Simpson composite si generalizzano ottenendo lal formmula di quadratura di Newton-Cotes di grado $k\in\{1,\hdots, 7, 9\}$, \textbf{a patto che $\boldsymbol n$ sia un multiplo di $\boldsymbol k$}. Tale formula è ottenuta approssimando

\begin{equation*}
	\begin{matrix}
			\int_{x_{(i-i)k}}^{x_{ik}} f(x) dx &\approx&  \frac{x_{ik}-x_{(i-i)k}}{k} \sum_{j=0}^k c_{jk} f(x_{(i-1)k+j}) \\
			&\equiv& \frac{kh}{k} \sum_{j=0}^k c_{jk} f_{(i-1)k+j} && i = 1,\hdots, \frac{n}{k},
	\end{matrix}
\end{equation*}
dove le ascisse $x_i = a + i \cdot h,\, i=1,\hdots, n$ rispettano le condizioni (\ref{eq:condizione_ascisse_equidistanti_newton_cotes}).

Pertanto, è definita la formula composita di grado $k$ su $n+1$ ascisse equidistanti (dove $n$ è multiplo di $k$) come segue:
\begin{definition}[Formula di quadratura composita]
	Sia $f\in C^{(k+\nu)}[a,b]$, allora la formula di quadratura composita di Newton-Cotes di grado $k\in\{1,\hdots, 7, 9\}$ con $n$ multiplo di $k$ è
	\begin{equation*}
    	I_k^{(n)}\equiv I_{kn}(f) = h \sum_{i=1}^{\frac{n}{k}} \sum_{j=0}^{k} c_{jk} f_{(i-1)k+j} .
	\end{equation*}
\end{definition}
...
\paragraph{Nota:} Dal punto di vista algoritmico è interessante ciò che sarà visto nell'implementazione dei casi più semplici, ovvero dei trapezi e di Simpson, con Matlab, tramite function ricorsive. In ogni caso è sfruttato il fatto che esiste un'espressione asintotica dell'errore quadratico.
...
\footnotetext{Dovuto a $\left(\frac{b-a}{2}\right)^2$ in (\ref{eq:errore_formula_trapezi_adattiva_n=2}) ed a $(b-a)^3$ in (\ref{eq:errore_formula_trapezi_adattiva_n=1}).}

\paragraph{Passo che genera la procedura adattiva:} è supposto di dover calcolare $I(f)$ con una accuratezza $\boldsymbol{tol}$ (parametro prefissato, ovvero scelto dall'utente) utilizzano la formula dei trapezi composita (\ref{eq:formula_trapezi_composita}) con $n=2$ e (\ref{eq:formula_trapezi}). Se $|E_1^{(2)}|\leq tol$ è terminata la procedura, altrimenti è riapplicata la stessa procedura sui due sottointervalli $\left[a,\frac{a+b}{2}\right],\, \left[\frac{a+b}{2},b\right]$, con tolleranza $\boldsymbol{tol/2}$.\\
