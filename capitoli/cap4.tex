...
(\ref{eq:P5_differenze_divise}) è  una proprietà algoritmica importante, è utilizzata per il calcolo efficiente della differenze divisa ed è possibile dimostrarla come segue.
...
\begin{theorem}\label{th:errore_interpolazione_forma_newt}\footnote{Slide 3 PDF 19, TH 4.3 PG 86.} 
...
\begin{remark}
    Il Teorema \ref{th:errore_interpolazione_forma_newt} definisce la forma dell'errore di interpolazione polinomiale (e di conseguenza è necessario impararlo).
\end{remark}
...
\begin{corollary}
    ...
    Utilizzando le ipotesi del Teorema \ref{th:errore_interpolazione_forma_newt} e supposto che $f\in C^{(n+1)}$ sul più piccolo intervallo contenente le ascisse in argomento alla differenza divisa in (\ref{eq:errInterFormaNewt}), allora (vedere (\ref{eq:P4DiffDiv})):
