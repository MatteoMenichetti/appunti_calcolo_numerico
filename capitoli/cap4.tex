...
L'interpolazione polinomiale è l'interpolazione di una serie di valori (ad esempio dei dati sperimentali) con una funzione polinomiale che passa per i punti dati. Sono utilizzati i polinomi per approssimare una funzione in quanto funzioni facilmente calcolabili.
...
\paragraph{N.B.:} Il Teorema permette di ridefinire il polinomio interpolante come (\ref{eq:polinomio_interpolante_forma_lagrange}), esprimendolo rispetto alla base di Lagrange, non facendolo variare nel risultato e quindi nell'unicità. Quindi, vale l'Osservazione \ref{rem:uguaglianza_polinomio_interpolante_lagrange}.
...
%\paragraph{Intermezzo utile per la definizione della "Interpolazione di Hermite"}\footnote{Slides 1-3 PDF 18.} Dato il polinomio interpolante in forma di Newton (\ref{eq:polinomio_interpolante_newton}) allora è possibile utilizzare l'Algoritmo \ref{alg:calcP(x)} per calcolare $p(x)$.
%
...
%
%L'obbiettivo è derivare un algoritmo per il calcolo delle derivate $\omega'(x)$ e $p'(x)$. Per questo vedere la Sezione \ref{sssec:calcolo_derivata_prima}. %Ciò significa che le righe precedenti si riducono a derivare un algoritmo, relativamente efficiente, per il calcolo delle derivate prime dei polinomi di base di Newton.
...
\subsubsection{Calcolo differenze divise polinomio di Hermite}
È possibile calcolare i coefficienti del polinomio, ovvero le differenze divise, con l'Algoritmo \ref{alg:differenze_divise_polinomio_hermite}, una variante dell'Algoritmo \ref{alg:calcolo_differenze_divise}, tenendo di conto di (\ref{eq:equivalenza_approx_f'_hermite}).
...
\paragraph{Nota sull'Algoritmo \ref{alg:calcolo_polinomio_interpolante}:} l'Algoritmo utilizza il fatto che le funzioni di base possano essere ottenute in modo iterativo, moltiplicando un termine di grado uno (il \gls{polinomio monico}). Pertanto, è meno efficiente dell'Algoritmo (di Horner generalizzato) \ref{alg:metodo_horner_modificato} in quanto effettua $4n\, flops$ ed utilizza una variabile di appoggio ($\omega$, la quale rappresenta $\omega_0,\,\omega_1,\,\omega_{i+1}$).
...
\paragraph{Costo computazionale Algoritmo \ref{alg:calcolo_sistema_tridiagonale_LU}}: Al posto di $\frac{2}{3}n^3$ flops c'è $3n$ ed al posto di $n^2\; 2n$. \textbf{Totale $\boldsymbol{8n}$ flops}, mentre se $A$ è simmetrica $7$ flops.
...
\paragraph{Spline cubica naturale:}
...
\paragraph{Spline cubica not-a-knot:} Imponendo  \textbf{le condizioni aggiuntive della spline not-a-knot} (\ref{eq:condizione_spline_cubica_not_a_knot}) e considerando gli estremi sinistro e destro con la notazione (\ref{eq:ampiezza_partizione}), le condizioni aggiuntive diventano
\begin{equation*}
    \frac{m_1-m_0}{h_1}=\frac{m_2-m_1}{h_2},\quad \frac{m_{n-1}-m_{n-2}}{h_{n-1}}=\frac{m_n-m_{n-1}}{h_n}
\end{equation*}
ovvero
\begin{equation*}
	h_2(m_1-m_0)=h_1(m_2-m_1),\, h_{n}(m_{n-1}-m_{n-2})=h_{n-1}(m_{n}-m_{n-1}).
\end{equation*}
Da queste sono ottenute le condizioni aggiuntive della spline cubica not-a-knot
...
\paragraph{Nota:} \textbf{Le condizioni aggiuntive della spline not-a-knot (\ref{eq:condizione_spline_cubica_not_a_knot2}) sono le condizioni da utilizzare per il calcolo pratico di una spline not-a-knot}.
...
\paragraph{Cosa si intende per \textit{migliore approssimazione} dei dati?} L'obbiettivo dell'approssimazione polinomiale nel senso dei minimi quadrati è stimare il polinomio del tipo (\ref{eq:polinomio_minimi_quadrati}). In forma vettoriale tale problema è risolto come segue: dati
\begin{equation}
	\underline{y}=
	\begin{bmatrix}
		f_0\\
...
$\text{Questo}_{\footnotemark}$ \textbf{non è più vero} se la matrice $D$ è $\text{rettangolare}_{\footnotemark}$ (perché (\ref{eq:sistema_quadrato}) e (\ref{eq:sistema_quadrato_2}) non avrebbero più la solita soluzione). 
