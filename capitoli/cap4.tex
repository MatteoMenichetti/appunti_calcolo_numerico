...
\begin{theorem}[Errore polinomio di Hermite]\label{th:errore_interpolazione_polinomio_hermite}
...
\begin{equation*}
    \begin{matrix}
        ...
        \sum_{i=0}^n|f_i-\Tilde{f}_i|\,|L_{in}(x)|&\leq&\left(\sum_{i=0}^n|L_{in}(x)|\right)\underbrace{\underset{i=0,\hdots,n}{\max}|f_i-\Tilde{f}_i|}_{\footnotemark}\\
...
\begin{algorithm}\caption{Calcolo delle ascisse di Chebyshev su un generico intervallo $[a, b]$.}
	\label{alg:cheby}
	\begin{lstlisting}[style=Matlab-editor]
...
\begin{definition}[Spline di $m$ su $\Delta$]\label{def:spline}
    ...
    \begin{enumerate}
        ...
        \item $s_m|_{[x_{i-1},x_i]}(x)\in\Pi_m,\quad \forall i=1,\hdots,n.$
    \end{enumerate}
\end{definition}
...
\end{enumerate}
Inoltre, una spline interpolante soddisfa le condizioni di interpolazione solite (ovvero $n+1$).

\begin{theorem}\label{th:n+m_condizioni_indipendenti}
...
\begin{definition}[Spline interpolante]\label{def:spline_interpolante}\footnote{Definizione 2 Slide 12 PDF 22, Definizione 4.4 PG 95. Ciò che è tra parentesi è un aggiunta maggiore chiarezza. Date le condizioni 1. e 2. della Definizione \ref{def:spline} è possibile dare la seguente definizione.}
...
\begin{definition}[Spline lineare]
...
La Definizione \ref{def:spline} di spline di grado $m$ prescinde dal fatto che la spline sia interpolante la funzione. Dato uno spazio vettoriale l'obbiettivo è quello di individuare una spline interpolante, possibilmente unica.
Quando il grado della spline aumenta allora esistono diverse spline di grado superiore, per le quali ciascuna sarà una spline di grado $m$ interpolante. Queste spline differescono di poco, dato che le prime $m$ condizioni sono imposte e solo le condizioni mancanti saranno imposte (ciascuna scelta riguardo le condizioni imposte genera spline diverse).

\paragraph{Nota:} La condizione 1. della Definizione \ref{def:spline} implica che la spline $s_m \in C^{(m-1)}[a,b]$. Questo requisito richiede di imporre delle condizioni sui punti interni della partizione $x_1,\hdots,x_{n-1}$, ovvero i punti di contiguità tra i sottointervalli $[x_{i-1},x_i]$ e $[x_i,x_{i+1}],$ per $ i=1,\hdots,n-1$. Pertanto, le condizioni da imporre sono le seguenti:
...
Sono esaminante \textbf{4 possibili implementazioni} delle condizioni aggiuntive e neccessarie da imporre per determinare una spline cubica interpolante, le quali sono aggiunte alle condizioni di interpolazione (\ref{eq:condizione_interpolazione_spline}).
\subsubsubsection{Spline cubica naturale}
\begin{definition}[Condizioni spline cubica naturale]
	\footnote{Slide 4 PDF 23, PG 96.} Una spline cubica $s_3(x)$ è detta naturale se impone le due condizioni aggiuntive
...
\begin{example}
	Calcolare i coefficienti $a,b,c,d$ in modo tale che
	\begin{equation*}
		s_3|_{[0,1]}(x) = x^3,\quad s_3|_{[1,2]}(x) = a x^3 + b x^2 + c x + d.
	\end{equation*}
	Sia un spline naturale cubica naturale
	\begin{equation*}
		s^{(i)}_3|_{[0,1]}(1) = s_3^{(i)}|_{[1,2]}(1),\quad i=0,1,2,
	\end{equation*}
	e
	\begin{equation*}
		s_3''|_{[1,2]}(2) = 0.
	...
	\begin{equation*}
		\begin{matrix}
			1 &=& a + b + c + d &\Rightarrow& \boldsymbol{d=} 1 - a - b - c \boldsymbol{= 2},\\
			3 &=& 3a + 2b + c &\Rightarrow& \boldsymbol{c=} 3-3a-2b \boldsymbol{=-6},\\
			6 &=& 6a + 2b\\
...
\subsubsubsection{Spline cubica completa}
\begin{definition}[Spline cubica completa]\footnote{Slide 4 PDF 23, PG 96.}
	La spline cubica $s_3(x)$ è detta completa se
...
\subsubsubsection{Spline cubica interpolante periodica} \footnote{Slide 4 PDF 23, PG 96.}
...
\begin{definition}[Spline cubica periodica]
	La spline cubica $s_3(x)$ e' periodica se
...
\subsubsubsection{Spline cubica interpolante not-a-knot}
...
È noto che nei primi due sottointervalli della partizione $\Delta$ siano presenti due polinomi di grado 3 (e così anche negli ultimi due). Ciò che è richiesto è che i due polinomi ed i primi due sottoinsiemi, tramite l'unione ($[x_0,x_1]\cup[x_1,x_2]=[x_0,x_1]$), coincidano. In questo modo è imposta una condizione in meno in modo esplicito. 
...
\begin{definition}[Spline cubica not-a-knot]
	La spline cubica $s_3(x)$ interpolante su $\Delta$ e' detta not-a-knot se le due condizioni aggiuntive sono 
...
Se è imposto che $\overset{\footnotemark}{p'''_1(x_1)}\overset{\footnotemark}{=}\overset{\footnotemark}{p'''_2(x_1)}$ allora $p_1(x)=s_3(x)=p_2(x),\, x\in [x_0,x_2]$. Quindi, in virtu' del Teorema \ref{th:grado_derivata_spline}, definita la spline $s_3(x)\in\Pi_3$ e le sue derivate come 
\begin{equation*}
    \begin{matrix}
