...
\begin{remark}
	...
	\end{equation*}
	oppure è possibile scriverlo come \gls{polinomio monico}, ovvero il polinomio con coefficiente del termine di grado più alto uguale ad 1, in forma esplicita come
...
\begin{theorem}[Polinomio interpolante in forma di Newton]\label{th:formaNewt}\footnote{Slide 5 PDF 16, TH 4.3 PG 80.}
...
\paragraph{Nota sulle ascisse di interpolazione:} Nel caso del polinomio interpolante di Newton, l'ascissa $x_n$ non e' utile ai fini del calcolo del polinomo.

\begin{example}
	\textbf{Calcolare la forma di Newton del polinomio interpolante le coppie di dati} $\boldsymbol{(x_i, f_i),\, i=0,1,2\; :\; (0,1),(2,-2), (3,4)}$.\\
...
Nel caso in cui siano fornite gia' le differenze divise allora e' possibile eliminare dall'Algoritmo \ref{alg:metodo_horner_modificato} la riga dove sono calcolate. Inoltre, e' possibile osservare che l'ascissa di interpolazione $n+1$-esima (vettore $x$ dell'Algoritmo) puo' non essere considerata perche' per calcolare la funzione di base di Newton l'ultima ascissa non utile. Per questo, verificare che $f$ e $x$ abbiano la stessa lunghezza significa condiderare $f-1$.
...
Inoltre e' possibile osservare che il calcolo di $\omega_i(x)$ come
\begin{equation*}
...
\begin{example}
	...
	\begin{equation}\label{eq:esempio_hermite}
		...
		\end{matrix}
	\end{equation}
	[E' possibile osservare che il polinomio interpolante di grado 1 la funzione $\sin(x)$ su $\{0,\pi\}$ e' il polinomio nullo (ovvero la prima riga di (\ref{eq:esempio_hermite})), invece il polinomio di interpolazione di Hermite, il quale fornisce l'interpolazione della derivata, fa si che l'approssimazione sia piu' accurata di quella nulla.]
	...
	\begin{center}
		...
		\end{tabular}
	\end{center}
	Pertanto, e' possibile scrivere esplicitamente il polinomio come:
	\begin{equation*}
		p(x) = x - \frac{1}{\pi} x^2 = \frac{x}{\pi}(\pi - x).
	\end{equation*}
	
	\noindent [Dalla tabella si nota che una differenza divisa con due argomenti uguali, come nella colonna 1, e' la derivata prima calcolata nel punto. E' possibile notare che
	\begin{enumerate}
		\item $f[0,0]$ e' ottenuta dalla combinazione delle due differenze divise $f[0]$,
		\item $f[0, \pi]$ e' ottenuta dalla combinazione di $f[0],f[pi]$,
		\item $f[\pi, \pi]$ e' ottenuta dalla combinazione di $f[\pi], f[pi]$,
		\item $f[0,0, \pi]$ e' ottenuto dalla combinazione di $f[0,0], f[0, \pi]$,
		\item $f[0,\pi, \pi]$ e' ottenuto dalla combinazione di $f[0,\pi], f[\pi, \pi]$,
		\item $f[0,0, \pi, \pi]$ e' ottenuto dalla combinazione di $f[0,0, \pi], f[0,\pi, \pi]$.
	\end{enumerate}
...
Al fine di studiare il condizioanmento dell'interpolazione polinomiale e' considerata una funzione $\Tilde{f}(x)$, perturbazione di $f(x)$, e costruito $\tilde{p}(x)$, il polinomio interpolante $\Tilde{f}(x)$ sulle ascisse di interpolazione assegnate, imponendo le condizioni di interpolazione (\ref{eq:polinomio_interpolante_f}), come
...
A questo scopo, dato che $\Lambda_n$ non dipende da un intervallo particolare $[a,b]$, e' fissato come $[-1,1]$ ed introdotta la seguente famiglia di polinomi:
...
\begin{property}[dei polinomi di Chebyshev di I specie]
	...
	\begin{itemize}
		...
		\begin{proof}
			È svolta per induzione su $k$. La tesi e' vera per $k=0$ e $k=1$.\\
...
Ragionamento analogo vale \textbf{per la spline cubica not-a-knot} ottenenuta imponendo le relative condizioni aggiuntive, per la quale \textbf{le condizioni (\ref{eq:condizione_spline_cubica_not_a_knot2}) e (\ref{eq:eqCalcMi}) danno origine al sistema lineare}
...
\begin{example}
	...
	\begin{equation}\label{eq:esempio_minimi_quadrati}
		...
		\end{bmatrix}.
	\end{equation}
	E' possibile osservare che la matrice dei coefficienti e' $7\times 2$ ed ha rango massimo, poiche' sono presenti almeno 3 ascisse distinte. Pertanto, la soluzione nel senso dei minimi quadrati di (\ref{eq:esempio_minimi_quadrati}) esiste ed e' unica. Pertanto il problema e' \gls{ben posto}.
\end{example}
