...
\paragraph{Divagazione su $\Pi_n$:}	$\boldsymbol{\Pi_n}$ \textbf{è lo spazio vettoriale dei polinomi di grado al più} $\boldsymbol n$. Uno spazio vettoriale è un insieme per il quale una combinazione lineare dei suoi elementi produce un elemento dello spazio stesso.
...
\paragraph{N.B.:} I polinomi $x^0,\, x^1,\, x^2,\, \hdots,\, x^n\in\Pi_n$ sono linearmente indipendenti e questi costituiscono la base canonica di $\Pi_n$. Pertanto, $\dim(\Pi_n)=n+1$. Questo significa che, per individuare univocamente un polinomio di $\Pi_n$,  saranno necessarie $n+1$ condizioni linearmente indipendenti.
 ...
 \begin{definition}[Trasposta Matrice di Vandermonde]
...
Il problema discreto (\ref{eq:sistema_lineare_vandermonde}), per la determinazione di (\ref{eq:condizioni_interpolazione_dimostrazione}), deriva dall'aver scelto la base delle potenze, ovvero ${x^0,x^1,\hdots, x^n}$, come base di $\Pi_n$.
...
\begin{theorem}[Polinomio interpolante in forma di Lagrange]
...
\begin{remark}\label{rem:uguaglianza_polinomio_interpolante_lagrange}
...
\begin{definition}[Polinomi di base di Newton]
...
\footnotetext{Moltiplicazione dell'elemento $\omega_{r-1}(x)$ per un \gls{polinomio monico}; $x_{r-1}$ è la $(r-1)$-esima ascissa di interpolazione.}
...
\paragraph{N.B.:} (\ref{eq:polinomio_base_newton2}) è una relazione ottenuta induttivamente dalla seconda riga del sistema (\ref{eq:polinomio_base_newton}) con gli zeri dei polinomi conosciuti. Questa relazione ricorsiva è utilizzata per il calcolo efficiente del polinomio.

\begin{theorem}[Polinomio interpolante in forma di Newton]\label{th:polinomio_forma_newton}\footnote{Slide 5 PDF 16, TH 4.3 PG 80.}
...
\end{theorem}
\begin{proof}
	\footnote{Dimostrazione riscorsiva del sistema (\ref{eq:famiglia_polinomi_interpolanti}) e di (\ref{eq:differenza_divisa}).}
    Per induzione la tesi è vera, con $r=0$, poiché $p_0(x)\equiv f_0\equiv f[x_0]$. Supposto vero per $r-1$ sarà dimostrato per $r$.
    
    \noindent Per ipotesi è supposto che
    \begin{equation*}
        p_{r-1}(x)\in\Pi_{r-1}:\boldsymbol{p_{r-1}(x_i)=f_i,\; i=0,\hdots r-1}
    \end{equation*}
    e che $p(x)$ sia definto come in (\ref{eq:famiglia_polinomi_interpolanti}). Sarà dimostrato quanto segue:
    \begin{enumerate}
        \item $p_r(x_i)=f_i, \; i=0,\hdots,r$ \footnotemark;
        \item $f[x_0, \hdots, x_r]$ è definita come in (\ref{eq:differenza_divisa}).
    \end{enumerate}
\footnotetext{Quindi è necessario dimostrare che $p_r$ è calcolabile come $p_{r-1}$.}
    \begin{proof}[Dimostrazione di 1.]
        ...
        È necessario dimostrare che la differenza divisa espressa come (\ref{eq:ridefDifDivisa}) coincide con (\ref{eq:differenza_divisa}). A questo fine è possibile osservare che $f[x_0, \hdots, x_r]$ è il coefficiente principale di $p_r(x)$.
    ...
    \stepcounter{footnote}
    \footnotetext{$\prod_{j=0,\\j\neq i}^r (x-x_j)$ è un \gls{polinomio monico} di grado $r$, è una produttoria di $r$ polinomi di grado 1.}
...
\begin{remark}[Forma di Newton del polinomio interpolante]\label{rem:polinomio_interpolante_forma_newton}\footnote{Slide 8 PDF 16.}
...
\begin{property}[Differenze divise]
...
\begin{example}[Esempio di utilizzo della proprietà (\ref{eq:P5_differenze_divise})]
...
\paragraph{Implementazione calcolo differenze divise:} Vedere Algoritmo \ref{alg:calcolo_differenze_divise}.
...
\paragraph{\textbf{Costo computazionale} dell'\textbf{Algoritmo (\ref{alg:calcolo_differenze_divise}})}:
    \begin{itemize}
        \item Flops: $\sum_{j=1}^{n}3(n-j+1)\overset{\footnotemark}{=} 3\sum_{j=1}^{n}j=3\cdot\frac{n(n+1)}{2}\boldsymbol{\approx \frac{3}{2}n^2\; flops}$; \footnotetext{Somma naturale.}
        \item Occupazione Memoria: \textbf{2 vettori} $\boldsymbol{(x, f)}$ ($x$ memorizza le ascisse interpolanti ed $f$ le differenze divise);
    \end{itemize}

\subsubsubsection{Metodo di Horner}
...
\paragraph{Implementazione algoritmo di Horner:}  Per derivare l'algoritmo è possibile osservare che
\begin{equation*}
	\begin{matrix}
...
\end{equation*}
\footnotetext{Trasformazione in $i=1$ perché $\omega_0\equiv 1$ (vedere (\ref{eq:polinomio_base_newton})).}
...
Inoltrepossibile osservare che il calcolo di $\omega_i(x)$ come
\begin{equation*}
    \omega_i(x)=\prod_{k=0}^{i-1}(x-x_k)
\end{equation*}
quindi
\begin{equation*}
	\omega'_i(x)=\sum_{k=0}^{i-1}\prod_{j=0,j\neq k}^{i-1}(x-x_j)
\end{equation*}
...
Data $f(x)\in C^{(1)}$ è possibile ottenere ciò che segue (vedere (\ref{eq:P4_differenze_divise})):
...
Conoscendo i valori $\{f'(x_i)\}$, mediante la Proprieta' (\ref{eq:P4_differenze_divise}), e' possibile calcolare le differenze divise come in precedenza (con la tabella triangolare inferiore),considerando le ascisse nella prima colonna come
\begin{equation*}
	a \leq x_0 = x_0 < x_1 = x_2 < \hdots < x_n = x_n \leq b,
\end{equation*}
invece di (\ref{eq:ascisse_interpolazione_hermite}).

È possibile concludere che se $x_{i+\frac{1}{2}}\rightarrow x_i,\, \forall i=\underbrace{0,\hdots, n}_{n+1}\Rightarrow\exists! p_{\boldsymbol H}(x)\in\Pi_{\boldsymbol{2n+1}}$ tale che
...
Pertanto, date le $n+1$ ascisse distinte (\ref{eq:ascisse_interpolazione}), $\exists!\, p_{\boldsymbol H}(x)\in\Pi_{2n+1}$, che soddisa le condizioni di interpolazione (\ref{eq:polinomio_interpolante_hermite}). Il polinomio $p_H(x)$ interpola la funzione $f(x)$ e la sua derivata, $f'(x)$, nelle ascisse di interpolazione.
...
\begin{remark}[Calcolo $p_H(x)$, caso generale]
    ...
    \end{equation}
    È possibile calcolare i coefficienti del polinomio, ovvero le differenze divise, con l'Algoritmo \ref{alg:calcDiffDivPolHer}, una variante dell'Algoritmo \ref{alg:calcolo_differenze_divise}, tenendo di conto di (\ref{eq:equivalenza_approx_f'_hermite}). Nell'Algoritmo, il vettore in ingresso, $\boldsymbol{f}$ contiene i valori $f(x_0), f'(x_0), f(x_1), f'(x_1),\hdots, f(x_n),f'(x_n)$ ed in uscita, essendo sovrascritto, le differenze divise in (\ref{eq:polinomio_hermite_base_newton}).
\end{remark}
\begin{algorithm}
...
\begin{example}
	...
	\begin{equation}\label{eq:esempio_hermite}
		...
		\end{matrix}
	\end{equation}
	[E' possibile osservare che il polinomio interpolante di grado 1 la funzione $\sin(x)$ su $\{0,\pi\}$il polinomio nullo (ovvero la prima riga di (\ref{eq:esempio_hermite})), invece il polinomio di interpolazione di Hermite, il quale fornisce l'interpolazione della derivata, fa si che l'approssimazione sia piu' accurata di quella nulla.]
	
	\noindent $p(x)\in\Pi_3$:
	\begin{equation*}
		p(x) = f[0]\cdot 1 + f[0,0]\cdot x + f[0,0,\pi]\cdot x^2 + f[0,0,\pi, \pi]\cdot x^2 (x-\pi).
	\end{equation*}
	...
	\begin{center}
		\begin{tabular}{c|cccc} 
			...
			$\pi$ & $f[\pi] = 0$ & $f[\pi, \pi] = 1$ & $\boldsymbol{f[0,\pi, \pi] = \frac{0-1}{\pi - 0} = -\frac{1}{\pi}}$ & $f[0, 0, \pi, \pi] = 0$
		\end{tabular}
	\end{center}
	...
	\noindent [Dalla tabella si nota che una differenza divisa con due argomenti uguali, come nella colonna 1,la derivata prima calcolata nel punto. E' possibile notare che
	\begin{enumerate}
		\item $f[0,0]$ottenuta dalla combinazione delle due differenze divise $f[0]$,
		\item $f[0, \pi]$ottenuta dalla combinazione di $f[0],f[pi]$,
		\item $f[\pi, \pi]$ottenuta dalla combinazione di $f[\pi], f[pi]$,
		\item $f[0,0, \pi]$ottenuto dalla combinazione di $f[0,0], f[0, \pi]$,
		\item $f[0,\pi, \pi]$ottenuto dalla combinazione di $f[0,\pi], f[\pi, \pi]$,
		\item $f[0,0, \pi, \pi]$ottenuto dalla combinazione di $f[0,0, \pi], f[0,\pi, \pi]$.
	\end{enumerate}
	Inoltre, dato che $f[0,0,\pi]$ e $f[0, \pi, \pi]$ sono uguali, in questo caso il polinomio $p$ avra' grado 2 (anche se potrebbe essere grado 3).]
\end{example}

E' possibile modificare leggermente l'algoritmo classico per il calcolo delle differenze divise (ovvero l'Algoritmo \ref{alg:calcolo_differenze_divise}), in modo che, dove sono presenti ascisse ripetute, sia utilizzato il corrispondente valore della derivata della funzione $f(x)$, come appena esposto. In questo modo sono presenti tutti gli elementi per implementare efficientemente il calcolo del polinomio interpolante di Hermite nella sua forma di Newton.
...
\begin{theorem}[Errore di interpolazione polinomiale]\label{th:errore_interpolazione_forma_newton}\footnote{Slide 3 PDF 19, TH 4.3 PG 86.} 
...
\begin{remark}[Errore polinomio di Hermite]\label{rem:errore_polinomio_hermite}
...
\begin{example}
	...
	\footnotetext{Simmetrica rispetto all'asse $y$ e massimo in $x=0$, vedere Figura \ref{fig:funzione_runge-5,5}.}
...
Al fine di studiare il condizioanmento dell'interpolazione polinomialeconsiderata una funzione $\Tilde{f}(x)$, perturbazione di $f(x)$, e costruito $\tilde{p}(x)$, il polinomio interpolante $\Tilde{f}(x)$ sulle ascisse di interpolazione assegnate, imponendo le condizioni di interpolazione (\ref{eq:polinomio_interpolante_f}), come
...
\paragraph{N.B.:} Studiare il condizionamento del problema significa stabilire in che modo piccole perturbazioni del problema, ovvero $||f-\Tilde{f}||$, si riflettano con perturbazioni sul risultato, ovvero su $||p-\Tilde{p}||$:
\begin{itemize}
...
\begin{equation}\label{eq:norma_infinito_g}
...
Dati (\ref{eq:polinomio_interpolante_f}) e (\ref{eq:polinomio_interpolante_perturbazione_f}), per $x\in [a,b]$, è ottenuto che
\begin{equation*}
    \begin{matrix}
        ...
        \sum_{i=0}^n|f_i-\Tilde{f}_i|\,|L_{in}(x)|&\leq&\underbrace{\left(\sum_{i=0}^n|L_{in}(x)|\right)}_{\lambda_n(x)}\underbrace{\underset{i=0,\hdots,n}{\max}|f_i-\Tilde{f}_i|}_{\footnotemark}\\
...
\noindent dove $\boldsymbol{\lambda_n(x)}$ è la \textbf{funzione di Lebesgue}. Tale funzione è positiva per definizione e dipende solo dalla scelta delle ascisse di interpolazione. Pertanto, dalla precedente disuguaglianza è ottenuto che
\begin{equation*}
...
\subsubsection{Connessioni tra condizionamento ed errore dell'interpolazione polinomiale}
...
\begin{definition}[Errore e polinomio di migliore approssimazione]\label{def:errore_polinomio_migliore_approssimazione}\footnote{Slide 7 PDF 20, Definizione 4.2 + Teorema 4.6 PG 90.}
...
È necessaria un'approssimazione dell'errore di migliore approssimazione (\ref{eq:errore_migliore_approssimazione}) in quanto l'errore crescerà al crescere di $n$, se la costante di Lebesgue diverge esponenzialmente. Per questo è fornita una maggiorazione per (\ref{eq:errore_migliore_approssimazione}), introducendo il modulo di continuità di una funzione.
...
\stepcounter{footnote}
\footnotetext{Definita nella dimostrazione del Teorema \ref{th:approssimazione_errore_migliore_approssimazione}.}
...
\subsection{Ascisse di Chebyshev}\label{ssec:ascisse_chebyshev}\footnote{PDF 21, PG 91-93.}
...
\begin{itemize}
    ...
    \item Dato l'errore di interpolazione (\ref{eq:errore_interpolazione_P4}), definito come 
    
    Segue la maggiorazione 
    \begin{equation*}
    	||e||\leq \frac{\overbrace{||f^{(n+1)}||}^{\footnotemark}}{(n+1)!}||\omega_{n+1}||,
    \end{equation*}
    la quale ha il membro $||\omega_{n+1}||$ che dipende dalla scelta delle ascisse di interpolazione
    \begin{equation*}
    	\omega_{n+1}(x)=\prod_{j=0}^n(x-x_j) = \{\phi\in\Pi_n: \phi \text{ \gls{polinomio monico}}\}.
    \end{equation*}
    Pertanto, è necessario capire come questa sia collegata con (\ref{eq:maggiorazione_errore_migliore_approssimazione}). 
\end{itemize}

Come conseguenza del Teorema \ref{th:Jackson} (di Jackson), l'obbiettivo di questa Sezione è quello di scegliere le ascisse di interpolazione (\ref{eq:ascisse_interpolazione}) in modo da minimizzare $||\omega_{n+1}||$. Per fare questo è necessario rendere la costante di Lebesgue $\Lambda_n$ abbia crescita ottimale con $n$ (ovvero $O(\log n)$).

E' necessario scegliere le ascisse di interpolazione $x_0,\hdots,x_n$ in modo tale da risolvere il \textbf{problema del minmax}, ovvero il seguente: \footnotemark
...
\end{equation}
con $w_{n+1}(x)=\prod_{j=0}^n(x-x_j)$ (calcolata come in (\ref{eq:polinomio_base_newton2})).
\addtocounter{footnote}{-1}
...
A questo scopo, dato che $\Lambda_n$ non dipende da un intervallo particolare $[a,b]$,fissato come $[-1,1]$ ed introdotta la seguente famiglia di polinomi:

\begin{definition}[Polinomi di Chebyshev di I specie]\label{def:polinomi_chebyshev_I_specie}
...
\begin{property}[dei polinomi di Chebyshev di I specie]
	...
	\begin{itemize}
		...
		\begin{equation}\label{eq:famiglia_polinomi_monici}
			...
			\end{cases}
		\end{equation}
		e' una \textbf{famiglia di polinomi monici} (perché $2^{1-k}$ è il reciproco del precedente coefficiente $2^{k-1}$);
		...
		con $T_k\left(\boldsymbol{x_j^{(k)}}\right)=0,\; j=0,\hdots,k-1,\;\text{dove } \boldsymbol{x_j^{(k)}}$ sono le \textbf{radici} di $\boldsymbol{T_k(x)}$. \footnote{Se le radici $x_j^{(k)}$ fossero tutte distinte e nell'intervallo $[-1,1]$, allora per il polinomio $\widehat T_k(x)$ è possibile scegliere un'ascissa di interpolazione.}
		 \item[P5)] Se $\boldsymbol{x=\cos\theta}$ allora $T_k(x)=T_k(\cos\theta)=\cos{(k\theta)},\quad\forall k\geq 0$. \footnote{Polinomio di Chebyschev, se $x$ è parametrizzata in modo idoneo.}
		\begin{proof}
			È svolta per induzione su $k$. La tesivera per $k=0$ e $k=1$.\\
		...
		\item[P8)] Gli zeri di $T_k(x)$ sono ottenuti imponendo che $x=\cos\theta,\;\theta\in [0,\pi]$, ovvero sono della forma
		\begin{equation*}
			x_i = \cos{\left(\frac{(2i+1)\pi}{2k}\right)},\quad i=0,\hdots,k-1,
		\end{equation*}
		\begin{proof}
				\begin{equation*}
				\begin{matrix}
...
\begin{algorithm}\caption{Calcolo delle ascisse di Chebyshev per costruire il polinomio interpolante di grado $n$ su un generico intervallo $[a, b]$.}
	...
	\begin{lstlisting}[style=Matlab-editor]
		...
		% alternativa: x = cos((2 * (n: -1 : 0) + 1) * pi / ( 2 * n + 2));
...
\end{equation*}
dove:
\footnotetext{Se $f$ e $p$ hanno grado $m$, significa che la costante di Lebesgue si trasforma in $\Lambda_m$, la quale è fissata, non cambia, ed al posto di $\frac{b-a}{m}$ è inserita la massima ampiezza dell'intervallo.}
\begin{itemize}
...
1. e 2. sono condizioni diverse: 1. definisce cos'è una spline di grado $m$ e 2. afferma che la spline coincide con il grado assegnato in ciascun sottointervallo. Una spline interpolante soddisfa le condizioni di interpolazione solite (ovvero $n+1$). È necessario imporre sulla 2. la condizione (\ref{eq:condizioni_interpolazione_spline_modificate}).
...
\begin{definition}[Condizioni di interpolazione]\label{def:condizioni_aggiuntive_spline}\footnote{Definizione 2 Slide 12 PDF 22, Definizione 4.4 PG 95. Ciò che è tra parentesi è un aggiunta maggiore chiarezza. Date le condizioni 1. e 2. della Definizione \ref{def:spline} è possibile dare la seguente definizione.}
    ...
    \begin{equation}\label{eq:condizione_interpolazione_spline}
...
\begin{equation}\label{eq:condizioni_interpolazione_spline_modificate}
    \begin{matrix}
...
\begin{definition}[Condizioni spline cubica naturale]
...
Dato che è richiesto che la spline cubica $s_3\in C^{(2)}[a,b]$, per la definizione di spline (Definizione \ref{def:spline}), è imposto che lo sia anche come funzione periodica. Analogamente a (\ref{eq:condizioni_spline_periodica}), sono imposte le seguenti condizioni aggiuntive:
...
Ciò che è stato appena definito nei riquadri è una conveniente espressione, in virtù del Teorema \ref{th:grado_derivata_spline}, delle condizioni aggiuntive della spline cubica not-a-knot, ovvero:
...
La rappresentazione delle condizioni (\ref{eq:condizione_spline_cubica_not_a_knot}) tramite le derivate seconde è utile al fine di definire l'unicità della spline interpolante. Dal punto di vista computazionale è importante che le condizioni siano imposte sulla derivata seconda.
...
\begin{equation}\label{eq:calcolo_mi}
    \begin{matrix}
        ...
        && && i=1,\hdots,n-1.
    \end{matrix}
\end{equation}\footnotetext{Per (\ref{eq:P5_differenze_divise}).} 
...
\textbf{Nel caso} di \textbf{spline naturali}, tenendo di conto delle condizioni (\ref{eq:condizione_spline_naturale}), è \textbf{ottenuto il sistema tridiagonale} 
\begin{equation*}
    \begin{pmatrix}
        2 & \xi_1 & && \\
        \varphi_2 & 2 & \xi_2&&\\
...
Ragionamento analogo vale \textbf{per la spline cubica not-a-knot} ottenenuta imponendo le relative condizioni aggiuntive, per la quale \textbf{le condizioni (\ref{eq:condizione_spline_cubica_not_a_knot2}) e (\ref{eq:calcolo_mi}) danno origine al sistema lineare}
...
\begin{definition}[Polinomio di approssimazione ai minimi quadrati di grado $m$]
	Il polinomio
	...
	\end{equation}\footnotetext{$n$ è tipicamente molto maggiore di $m+1$, quindi il numero di misurazioni deve essere almeno pari al numero di coefficienti. Se $n=m+1$, ovvero $n$ è pari al numero di coefficienti, allora è calcolata l'interpolazione.}
\end{definition}

\begin{remark}[Condizione di esistenza ed unicità del polinomio ai minimi quadrati]\label{rem:ascisse_distinte_polinomio}
...
Tuttavia, è possibile esprimere $\underline{z}$ come
\begin{equation*}
...
$\overset{\footnotemark}{\text{Questo}}$ \textbf{non è più vero} se la matrice $D$ è $\overset{\footnotemark}{\text{rettangolare}}$ (perché (\ref{eq:sistema_quadrato}) e (\ref{eq:sistema_quadrato_2}) non avrebbero più la solita soluzione). 

(\ref{eq:sistema_quadrato_2}) è utilizzato, nella pratica, calcolando $D=diag(\underline{p})$, con $\underline{p}$ vettore contenente una \textbf{distribuzione di probabilità discreta} ($\iff \underline{p}\geq 0 \,\land\, \text{sum}(\underline{p})=1$), per dare un diverso peso, $p_i$, a ciascuna equazione.
...
\subsection{Risoluzione di un sistema tridiagonale}\label{ssec:risoluzione_sistema_tridiagonale}
