...
Data $f(x)\in C^{(1)}$, se (\ref{eq:x_i/2->x_i}) segue che (vedere (\ref{eq:P4_differenze_divise})):
...
\paragraph{Calcolo differenze divise polinomio di Hermite:}{È possibile calcolare i coefficienti del polinomio, ovvero le differenze divise, con l'Algoritmo \ref{alg:differenze_divise_polinomio_hermite}, una variante dell'Algoritmo \ref{alg:calcolo_differenze_divise}, tenendo di conto di (\ref{eq:equivalenza_approx_f'_hermite}).

\noindent Inoltre, è possibile calcolare $p_H(x)$ tramite una versione modificata dell'algoritmo di Horner generalizzato (Algoritmo \ref{alg:metodo_horner_modificato}), nella quale è utilizzato un vettore delle ascisse $x=[x_0\, x_0\, x_1\, x_1\, \hdots\, x_{n-1}\, x_{n-1}\,x_n]$, a patto di conoscere le differenze divise (ovvero i coefficienti del polinomio). Dato che le ascisse sono duplicate, nel ciclo implementato per il calcolo del polinomio, sarà contato fino ad $2n+1$. Il $+1$ è dovuto al fatto che $x_n$ è ripetuta solo una volta.}
...
\begin{example}
	...
	\noindent [Dalla tabella si nota che una differenza divisa con due argomenti uguali, come nella colonna 1,la derivata prima calcolata nel punto. È possibile notare che
	\begin{enumerate}
	...
	\end{enumerate}
	Inoltre, dato che $f[0,0,\pi]$ e $f[0, \pi, \pi]$ sono uguali, in questo caso il polinomio $p$ avrà grado 2 (anche se potrebbe essere grado 3).]
\end{example}
...
\begin{definition}[Errore interpolazione polinomiale]
...
\paragraph{N.B.:} è possibile osservare che $f(x)=p(x)+e(x)$, con $e(x)$ errore commesso nell'approssimazione di $f(x)$ tramite il polinomio interpolante $p(x)$.

\begin{definition}[Errore massimo di interpolazione]
...
\begin{theorem}[Errore di interpolazione polinomiale polinomio di Newton]\label{th:errore_interpolazione_forma_newton}\footnote{Slide 3 PDF 19, TH 4.3 PG 86.} 
...
\begin{corollary}
    ...
    Dato $p(x)$, polinomio interpolante $f(x)$ in forma di Newton, definito come (\ref{eq:polinomio_interpolante_newton}), sulle ascisse distinte (definite come in (\ref{eq:ascisse_interpolazione})) e supposto che $f\in C^{(n+1)}(\min\{\underset{i}{\min}\,x_i,\,x\},\, \max\{\underset{i}{\max}\,x_i,\, x\})$ sul più piccolo intervallo contenente le ascisse in argomento alla differenza divisa in (\ref{eq:errore_interpolazione_forma_newton}), allora (vedere (\ref{eq:P4_differenze_divise})):
...
Dati (\ref{eq:condizioni_interpolazione_condizionamento}) e (\ref{eq:polinomio_interpolante_perturbazione_f}), per $x\in [a,b]$ (ascisse di interpolazione), è ottenuto che
\begin{equation*}
    \begin{matrix}
        ...
        && &=&\boldsymbol{\boldsymbol{\lambda_n(x)}\cdot\,||f-\Tilde{f}||},
    \end{matrix}
\end{equation*}
...
Ad ogni ascissa di interpolazione $x_i\in [a,b]$ corrisponde
...
\stepcounter{footnote}
\footnotetext{Per $(\ref{eq:xi})\overset{\text{e}}{+}(\ref{eq:xi_i})$.}
...
\begin{property}[di $\Lambda_n$]
	\begin{itemize}
		...
		\item[P4)] la scelta di ascisse equidistanti da una costante di Lebesgue genera una successione $\{\Lambda_n\}$, la quale diverge esponenzialmente come $\Lambda_n\approx 2^n$ con $n\rightarrow\infty$. Pertanto, la scelta di ascisse equidistanti non è, in generale, appropriata.
	\end{itemize}
\end{property}
...
\paragraph{$\underset{p\in \Pi_n}{\arg\min}f(x)$} In generale $\underset{x\in X}{\arg\min}f(x)\{x\,|\,\forall y : f(y)\geq f(x)\}$, è l'insieme dei valori di $x$ per i quali $f(x)$ raggiunge il suo più alto valore $M$.
...
La Definizione \ref{def:errore_polinomio_migliore_approssimazione} di errore e polinomio di migliore approssimazione denota l'esistenza di una funzione con un minimo sull'intervallo $[a,b]$ e fissa il grado $n$, ovvero il grado dell'insieme di polinomi ($\Pi_n$) con i quali $f$ sarà approssimata.
