...
In molte applicazioni è richiesto di approssimare una funzione $f:[a,b]\rightarrow\mathbb R$, in quanto questa potrebbe essere troppo complessa o non nota. Per la funzione sono conosciute le ascisse (tra loro \underline{distinte}):
\begin{equation}\label{eq:ascisse_interpolazione}
	a\leq x_0<x_1<\hdots<x_n\leq b,\quad x_i\neq x_j, \quad i\neq j, \quad \forall\, i,j=0,\hdots,n.
\end{equation}

Con (\ref{eq:ascisse_interpolazione}) sono rappresentate $n+1$ ascisse distinte.
...
\begin{theorem}[Esistenza ed Unicità del polinomio interpolante]\label{th:esistenza_unicita_polinomio_interpolante}\footnote{Slide 2 PDF 15, TH 4.1 PG 78}
    Date le ascisse distinte del tipo (\ref{eq:ascisse_interpolazione}), $\exists!\, p(x)\in\Pi_n: p(x_i)=f_i,\, i=0,\hdots,n$ (ovvero esiste unico polinomio che soddisfa le condizioni (\ref{eq:condizioni_interpolazione})).
\end{theorem}
\begin{proof}
...
\footnotetext{. Le ascisse $x_0, \hdots, x_n$ soddisfano (\ref{eq:ascisse_interpolazione}).}
...
\begin{definition}
    \textbf{(\ref{eq:polinomio_interpolante_forma_lagrange})} costituisce la \textbf{forma di Lagrange} del polinomio interpolante.
\end{definition}

La precedente Definizione significa che (\ref{eq:polinomio_interpolante_forma_lagrange}) è espresso rispetto alla base di Lagrange, anche se il polinomio interpolante non varia.
...
\begin{theorem}[Forma di Newton]\label{th:formaNewt}\footnote{Slide 5 PDF 16, TH 4.3 PG 80.}
    ...
    \end{equation}
\end{theorem}
\begin{proof}\footnote{Dimostrazione riscorsiva del sistema (\ref{eq:famiglia_polinomi_interpolanti}) e di (\ref{eq:differenza_divisa}).}
...
(\ref{eq:P5_differenze_divise}) è  una proprietà algoritmica importante, è utilizzata per il calcolo efficiente della differenze divise.
...
\paragraph{N.B.:} Il Teorema \ref{th:errore_interpolazione_forma_newton} definisce la forma dell'errore di interpolazione polinomiale (e di conseguenza è necessario impararlo).
