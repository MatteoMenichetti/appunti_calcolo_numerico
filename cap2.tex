...
\begin{definition}[Radice con molteplicità $m$]\label{def:radiciMultiple}
...
\begin{definition}[\textbf{Ordine di convergenza} di un metodo iterativo] \label{def:ordConv}
...
\begin{example} [Controesempio di non convergenza]
    ...
    \end{equation*}
    La successione di approssimazioni ottenuta non converge ad alcuna radice (ed $x_0$ non è una radice).
\end{example}
...
\begin{algorithm}
\caption{Implementazione del metodo delle secanti.}\label{alg:metSec}
    \begin{lstlisting}[style=Matlab-editor]
    function x = secanti(f, f1, x0, tol, itmax)
    fx = feval(f, x0);
    f1x = feval(f1, x0);
    x = x0 - fx/f1x;
    for i = 1 : itmax
    ...
    if abs(x-x0) > tolx, warning('metodo non converge'), end
    \end{lstlisting}
\end{algorithm}
